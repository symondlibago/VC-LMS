  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setIsBorrowModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white h-11 rounded-xl px-6">
      ...
      <Card className="border-none shadow-lg rounded-2xl overflow-hidden">
        <CardContent className="p-0">
          {loading ? <div className="p-20 flex justify-center"><Loader2 className="animate-spin text-indigo-900 h-8 w-8" /></div> : (
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {activeBorrowings.map(record => {
                    ...
                    <tr key={record.id} className={`hover:bg-gray-50 transition-colors ${overdue ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="p-4">
                         <div className="font-bold text-indigo-900 truncate max-w-[200px]">{record.book?.title || 'Unknown Book'}</div>
